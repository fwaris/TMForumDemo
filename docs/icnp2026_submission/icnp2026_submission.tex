...
This shift creates a safety and reliability problem: natural-language or weakly structured intents are attractive because they are easy to express, but they can be ambiguous or under-specified. How to ensure NL intents are complete and sound? This paper specifically addresses this question and in doing so addresses the broader question of how to extract trusted, deterministic content from non-deterministic human and/or Large Language Model (LLM) generated content over an expansive domain of discourse.

This paper presents a prototype, high-assurance implementation of the TM Forum TMF921 Intent Management API standard through which autonomous-network control intents are to be submitted. TM Forum is a telecom standards body that defines a large surface area of telecom operation APIs. TMF921 imposes the minimal semantic requirements through an OWL ontology. However any specific telecom provider implementation of TMF921 will have additional requirements that are not covered by the ontology. We also demonstrate provider-specific intent validation via further refinements of the ontology-projected artifacts.

This paper shows how NL intents can be trusted through proof-relevant guarantees provided by formal verification systems based on dependently typed languages. First (offline) the OWL ontology is projected into dependent types of the F* language. Then additional refined types are added to capture provider-specific constraints. These serve as propositions against which incoming intents are checked. Then (online) received natural-language intents are translated by a LLM to F* code that is validated (type-checked) against the ontology-derived dependent types using the F* proof checker. Only valid intents are admitted.

Can contemporary LLMs generate intent-capturing F* code reliably enough to be practical? We observed that valid intents can be processed with high reliability after only mild prompt tuning. On a 300-trial evaluation over ten semantically correct expressions, GPT-5.4 achieved 100\% first-attempt success in generating F* artifacts that passed the proof-oriented checks.
\end{abstract}
...
This paper describes a prototype implementation of the TM Forum \cite{tmforum_site} TMF921 Intent Management API \cite{tmf921_api_page} that addresses this admission problem. TMF921 allows for intent submission in natural language and JSON-structured formats. Here the focus is on the natural-language path.

First, offline, the TM Forum intent ontology is projected into F* \cite{fstar_site,fstar_popl} dependent types that capture the standards-aligned semantics of the ontology \cite{tr290a,tr292d}. This is a partial projection enough to demonstrate the ideas discussed in this paper. Then additional F* types are added that are refinements of the ontology-projected types, which capture additional constraints of a hypothetical telecom provider. Then, online, incoming requests at the TMF921 boundary are normalized and, for the natural-language path, translated by an LLM into F* artifacts that must type-check against that generated types before admission. Around that checked path, the system emits a canonical intent intermediate representation (IR), normalized JSON-LD \cite{jsonld11}, generated F* modules and other items for traceability and diagnostics.
...
\begin{enumerate}
\item It establishes dependent-type, proof-oriented validation as an admission-layer pattern for ensuring high-trust LLM outputs. This approach is demonstrated in the context of autonomous-network intent validation via a TMF921-compliant API \cite{tmf921_spec,tr292_tio}.
\item It demonstrates the expressive power of dependently typed languages for domain modeling. Provider-specific constraints are naturally captured through successive refinement of TMF921 ontology-derived types. More broadly, dependent types enable a level of compositionality and semantic precision not attainable with conventional strongly typed languages, business rules engines, or schema validation approaches such as JSON Schema. While OWL provides limited compositional constructs, it lacks equivalent guarantees of executable validation.
\end{enumerate}
...
To illustrate, Listing~\ref{lst:dependent-type-sketch} starts with an ordinary function type and then adds a dependent type whose proposition depends on the value being checked.

\begin{lstlisting}[style=fstar,caption={Small F* dependent-type sketch.},label={lst:dependent-type-sketch}]
// ordinary function type
type T1 = nat -> nat
let addOne : T1 = fun x -> x + 1

// dependent proposition over a value
type Even (n:nat) = _:unit { n % 2 = 0 }

// input and result must both satisfy Even
type T2 = (x:nat{Even x}) -> nat{Even result}
let addTwo : T2 = fun x -> x + 2

// This would fail: it produces an odd result.
// let addOneError : T2 = fun x -> x + 1
\end{lstlisting}

Since \texttt{T1} is \textit{inhabited} via the function \texttt{addOne}, it means that \texttt{addOne} is a proof of \texttt{T1}. So under Curry--Howard the type \texttt{T1} can be read as a proposition that is proven by the program \texttt{addOne}. The type \texttt{Even n} is a dependent type because it \textit{depends} on the \textit{value} \texttt{n} which is supplied from where the type is used. Here \texttt{n} in \texttt{Even n} is used in the constraint \texttt{n \% 2 = 0} that is part of the type declaration. In fact any arbitrary constraint can be used as part of a type declaration.
...
The F* type checker enables static, proof-oriented validation of program correctness and is capable of handling significantly greater complexity than that required for TMF921 intent validation. Although the intents are declarative and relatively simple, they ultimately drive complex \textit{control code} for managing dynamic networks. This suggests a possible extension in which LLM-based generation is applied beyond intent parsing to the synthesis of \textit{control code}. Type-based validation could then be enforced at finer levels of granularity, supporting compositional constraints and enabling intent expressions to accommodate previously unanticipated scenarios.

Type-checked F* programs can be extracted to other languages for practical execution. One such target is F\#, a general-purpose functional programming language \cite{fsharp}. In our implementation, the TMF921 Web API and shell processing are written in F\#, which internally invokes F* for validation. Both F* and F\# share a common lineage in the ML family of languages \cite{milner_ml_polymorphism}. F\# can be viewed as a non-dependently typed counterpart to F*, making it a suitable execution target for LLM-generated, F*-verified \textit{control code}.
...
Figure~\ref{fig:ontology-fragment} shows the compact TMF921/TIO derived fragment used to interpret this request. The fragment is representative but not a precise depiction. The diagram is standards-oriented rather than provider-oriented: it contains the TMF921 intent resource and expression envelope, then the ontology-level target, service class, expectations, quantities, reporting, and security posture needed by the Detroit Stadium demo. It deliberately excludes the provider profile, capacity ceiling, and local admission thresholds, which are introduced only as refinements in Listing~\ref{lst:provider-refinement}.
...
\begin{tikzpicture}[
  ...
  api/.style={box, fill=blue!7},
  tio/.style={box, fill=green!7},
  exp/.style={box, fill=orange!10},
  q/.style={box, fill=purple!7},
  arrow/.style={-Latex, thick}
]

\node[api] (resource) at (0,4.2)
  {TMF921 Intent\\resource\\\texttt{Intent}};
\node[api] (envelope) at (0,3.0)
  {TMF921 expression\\envelope\\\texttt{expression}};
\node[tio] (expr) at (0,1.8)
  {TIO intent expression\\\texttt{icm:IntentExpression}};

\node[tio] (target) at (-6.2,0.25)
  {Target descriptor\\\texttt{icm:Target}\\{\tiny Detroit Stadium / venue}};
\node[tio] (service) at (-2.1,0.25)
  {Service class\\\texttt{icm:Service}\\{\tiny premium 5G broadcast}};
\node[exp] (expectation) at (2.1,0.25)
  {Expectation set\\\texttt{log:allOf}};
\node[tio] (security) at (6.2,0.25)
  {Security posture\\\texttt{TR292I}\\{\tiny emergency traffic preserved}};

\node[exp] (delivery) at (-2.1,-1.35)
  {Delivery\\expectation\\{\tiny Apr. 25, 2026}\\{\tiny 18:00--22:00}};
\node[exp] (property) at (2.1,-1.35)
  {Property\\expectation\\metric condition};
\node[exp] (reporting) at (6.2,-1.35)
  {Reporting\\expectation\\{\tiny 60-min reports}\\{\tiny immediate alerts}};
\node[q] (quantity) at (2.1,-2.65)
  {Quantity\\\texttt{quan:Quantity}\\{\tiny 200 devices}\\{\tiny latency $\leq 20$ ms}};

\draw[arrow] (resource) -- (envelope);
\draw[arrow] (envelope) -- (expr);
\draw[arrow] (expr) -- (target);
\draw[arrow] (expr) -- (service);
\draw[arrow] (expr) -- (expectation);
\draw[arrow] (expr) -- (security);
\draw[arrow] (expectation) -- (delivery);
\draw[arrow] (expectation) -- (property);
\draw[arrow] (expectation) -- (reporting);
\draw[arrow] (property) -- (quantity);

\end{tikzpicture}%
}
\caption{TMF921/TIO ontology fragment for the Detroit Stadium intent. The nodes are standards-level concepts carried by the TMF921 intent resource and expression field; the small text shows demo values that inhabit those slots. Provider profiles and admission thresholds are not part of this ontology fragment.}
\label{fig:ontology-fragment}
\end{figure*}

Listing~\ref{lst:common-core} gives a simplified F* projection of the same standards-level ontology fragment. The request must name a target and service, provide a complete delivery window and reporting expectation, and attach positive measurable quantities. The listing stops at \texttt{measured\_tmf921\_intent}; no provider profile, venue-specific limit, or local admission policy appears in this type layer.

\begin{lstlisting}[style=fstar,caption={Simplified TMF921/TIO F* types for the ontology fragment.},label={lst:common-core}]
open TR290A
open TR292G
open TR292I

type tmf921_target_kind =
  | VenueTarget
  | FacilityTarget

type tmf921_intent_expression = {
  target:target_descriptor tmf921_target_kind;
  service_class:option string;
  delivery:delivery_window;
  metrics:metric_requirements;
  reporting:reporting_expectation;
  security:security_posture
}

type tmf921_intent = {
  intent_name:string;
  expression:tmf921_intent_expression
}

let positive (v:option nat) : bool =
  match v with | Some n -> n > 0 | None -> false

let ontology_complete (i:tmf921_intent) : bool =
  let e = i.expression in
  target_present e.target &&
  service_present e.service_class &&
  delivery_window_present e.delivery &&
  reporting_expectation_present e.reporting &&
  required_metrics_present false e.metrics

let quantities_ok (i:tmf921_intent) : bool =
  let m = i.expression.metrics in
  positive m.metric_primary_device_count &&
  positive m.metric_latency_bound_ms &&
  positive m.metric_reporting_interval_minutes

type actionable_tmf921_intent =
  i:tmf921_intent { ontology_complete i }

type measured_tmf921_intent =
  i:actionable_tmf921_intent { quantities_ok i }
\end{lstlisting}

Provider checks are then layered on top of the TMF921/TIO types. In the running live-broadcast example, Detroit Stadium resolves to the \texttt{LiveBroadcastGold} profile, which admits at most 250 production devices, requires a latency bound no tighter than 20 ms, and accepts the requested 60-minute reporting interval. These constraints are not part of the standards-level ontology; they are local refinements over \texttt{measured\_tmf921\_intent}.

\begin{lstlisting}[style=fstar,caption={Provider-specific F* refinement types over the TMF921 types.},label={lst:provider-refinement}]
type provider_profile =
  | LiveBroadcastGold
  | UnsupportedProfile

let resolve_profile
  (i:measured_tmf921_intent) : provider_profile =
  match i.expression.target.descriptor_name with
  | Some "Detroit Stadium" -> LiveBroadcastGold
  | _ -> UnsupportedProfile

let profile_matches
  (p:provider_profile)
  (i:measured_tmf921_intent) : bool =
  resolve_profile i = p && p <> UnsupportedProfile

let provider_capacity_ok
  (p:provider_profile)
  (i:measured_tmf921_intent) : bool =
  match p, i.expression.metrics.metric_primary_device_count with
  | LiveBroadcastGold, Some n -> n <= 250
  | _, _ -> false

let provider_latency_ok
  (p:provider_profile)
  (i:measured_tmf921_intent) : bool =
  match p, i.expression.metrics.metric_latency_bound_ms with
  | LiveBroadcastGold, Some ms -> 20 <= ms
  | _, _ -> false

let provider_reporting_ok
  (p:provider_profile)
  (i:measured_tmf921_intent) : bool =
  match p, i.expression.reporting.expected_interval_minutes with
  | LiveBroadcastGold, Some minutes -> 15 <= minutes
  | _, _ -> false

let provider_policy_ok
  (i:measured_tmf921_intent) : bool =
  let s = i.expression.security in
  i.expression.reporting.requires_immediate_alerts &&
  s.declared_safety_policy &&
  s.preserves_emergency_traffic &&
  not s.requests_public_safety_preemption

type profiled_intent (p:provider_profile) =
  i:measured_tmf921_intent { profile_matches p i }

type provider_checked_intent (p:provider_profile) =
  i:profiled_intent p {
    provider_capacity_ok p i &&
    provider_latency_ok p i &&
    provider_reporting_ok p i &&
    provider_policy_ok i
  }
\end{lstlisting}
...
The current evidence is architectural rather than broad empirical. The natural-language handling is narrow, the provider model is simplified, and the evaluation covers limited scenario families. The next steps are to broaden the scenario set, connect admission checks to dynamic state via \textit{control code} generation to understand the power and limits of LLM-generated but proof-validated code.
